        ...
        try:
            ...
            async def _do_publish():
                try:
                    ...
                    logger.info(
                        f"[REDIS_PUBLISH] Published document status update: document_id={document_id} status={status} channel={channel}"
                    )
...
def extract_text_activity(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"[DOC_ACTIVITY] Document {document_id} does not exist (may have been deleted)"
            )
            return {
                "success": False,
        ...
        logger.info(
            f"[DOC_ACTIVITY] Text extraction completed for document_id={document_id}, text_length={len(text)}"
        )
        ...
        logger.error(
            f"[DOC_ACTIVITY] Text extraction failed for document_id={document_id}: {e}",
            exc_info=True,
        )
        return {
            "success": False,
...
def chunk_text_activity(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[DOC_ACTIVITY] Text chunking completed for document_id={document_id}, chunks={len(chunk_objects)}"
        )
        ...
        logger.error(
            f"[DOC_ACTIVITY] Text chunking failed for document_id={document_id}: {e}",
            exc_info=True,
        )
        return {
            "success": False,
...
def embed_chunks_activity(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not api_key_ctx.openai_api_key:
            raise ValueError("OpenAI API key is required for embeddings")
        embeddings_client = OpenAIEmbeddingsClient(api_key=api_key_ctx.openai_api_key)
        ...
        logger.info(
            f"[DOC_ACTIVITY] Generated {len(embeddings)} embeddings for document_id={document_id}"
        )
        ...
        logger.error(
            f"[DOC_ACTIVITY] Embedding generation failed for document_id={document_id}: {e}",
            exc_info=True,
        )
        return {
            "success": False,
...
def upsert_vectors_activity(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"[DOC_ACTIVITY] Vector upsert completed for document_id={document_id}"
        )
        ...
        logger.error(
            f"[DOC_ACTIVITY] Vector upsert failed for document_id={document_id}: {e}",
            exc_info=True,
        )
        return {
            "success": False,
...
def update_document_status_activity(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        f"[DOC_ACTIVITY] Updating document status for document_id={document_id} to {status}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"[DOC_ACTIVITY] Document {document_id} does not exist (may have been deleted), skipping status update"
            )
            return {
                "success": True,
                "skipped": True,
        ...
        if status == Document.Status.READY:
            try:
                extracted_text = DocumentText.objects.get(document=document)
                chunking_config = ChunkingConfig()
                document.tokens_estimate = count_tokens(
                    extracted_text.text, chunking_config.tokenizer_model
                )
                update_fields.append("tokens_estimate")
            except DocumentText.DoesNotExist:
                logger.warning(
                    f"[DOC_ACTIVITY] DocumentText not found for document_id={document_id}, skipping token count"
                )
        ...
        logger.info(
            f"[DOC_ACTIVITY] Document status updated for document_id={document_id} to {status}"
        )
        ...
        logger.error(
            f"[DOC_ACTIVITY] Failed to update document status for document_id={document_id}: {e}",
            exc_info=True,
        )
        return {
            "success": False,
...
def check_and_publish_queue_complete_activity(user_id: int) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"[DOC_ACTIVITY] Checking if all documents are complete for user_id={user_id}"
    )
    ...
    try:
        ...
        if total_count == 0:
            ...
            logger.debug(
                f"[DOC_ACTIVITY] No documents found for user_id={user_id}, skipping queue_complete check"
            )
            return {
                "success": True,
                "published": False,
        ...
        if complete_count == total_count:
            logger.info(
                f"[DOC_ACTIVITY] All {total_count} documents complete for user_id={user_id}, publishing queue_complete"
            )
            ...
            try:
                ...
                async def _do_publish():
                    try:
                        ...
                        if not redis_client:
                            logger.warning(
                                f"[DOC_ACTIVITY] Redis client unavailable, cannot publish queue_complete for user_id={user_id}"
                            )
                            return
                        ...
                        await redis_client.publish(channel, event_json.encode("utf-8"))
                        logger.info(
                            f"[DOC_ACTIVITY] Published queue_complete event for user_id={user_id} to channel {channel}"
                        )
                    except Exception as e:
                        logger.error(
                            f"[DOC_ACTIVITY] Failed to publish queue_complete for user_id={user_id}: {e}",
                            exc_info=True,
                        )
        ...
        logger.error(
            f"[DOC_ACTIVITY] Failed to check queue completion for user_id={user_id}: {e}",
            exc_info=True,
        )
        return {
            "success": False,

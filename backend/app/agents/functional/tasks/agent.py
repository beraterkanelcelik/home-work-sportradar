...
def execute_agent(
    ...
    try:
        logger.info(
            f"[EXECUTE_AGENT] Starting {agent_name} agent with {len(messages)} messages"
        )
        ...
        needs_summarization = check_summarization_needed_task(
            messages=messages, token_threshold=40000, model_name=model_name
        ).result()
        ...
        if context_usage.get("usage_percentage", 0) > 80.0:
            logger.info(
                f"Context usage {context_usage['usage_percentage']:.1f}% exceeds 80%, trimming messages"
            )
        ...
        try:
            ...
            record_context_usage(
                model_name or OPENAI_MODEL, context_usage.get("usage_percentage", 0)
            )
        except Exception:
            pass
        ...
        logger.info(
            f"[EXECUTE_AGENT] {agent_name} completed: context={context_usage['usage_percentage']}%"
        )
...
def refine_with_tool_results(
    ...
    try:
        logger.info(
            f"[REFINE] Starting for agent={agent_name}, messages_count={len(messages)}, tool_results_count={len(tool_results)}"
        )

...
def execute_tools(
    ...
    try:
        logger.info(
            f"[EXECUTE_TOOLS] Starting execution of {len(tool_calls)} tools for agent={agent_name}"
        )

        # Get agent for tools
        agent = get_agent(agent_name=agent_name, user_id=user_id, api_key=api_key)
        ...
        for tool_msg in tool_messages:
            ...
            results.append(
                ToolResult(
                    tool=tool_msg.name,
                    args=matching_tool_call.get("args", {})
        ...
        logger.info(
            f"[EXECUTE_TOOLS] Completed execution: {len(results)} results returned"
        )
        return results

...
def create_agent_with_summarization(
    ...
    try:
        ...
        summary_model = (
            "gpt-4o-mini" if "mini" not in model_name.lower() else model_name
        )

        api_key = None
        if hasattr(agent, "_api_key"):
            api_key = agent._api_key
        if not api_key:
            logger.debug("No API key available for summarization, skipping middleware")
            return agent
        ...
        if hasattr(agent, "with_config"):
            try:
                ...
                logger.debug(
                    f"Wrapped agent with SummarizationMiddleware (trigger: 4000 tokens, keep: 20 messages)"
                )
                return wrapped_agent
            except Exception as e:
                logger.debug(
                    f"Agent doesn't support middleware wrapping: {e}, using agent as-is"
                )
                return agent
        else:
            ...
            logger.debug(
                "Agent doesn't support with_config, summarization handled by checkpointing"
            )
            return agent

    except ImportError:
        logger.debug(
            "SummarizationMiddleware not available (langchain.agents.middleware), using agent as-is"
        )
        return agent
    except Exception as e:
        logger.warning(
            f"Error setting up SummarizationMiddleware: {e}, using agent as-is"
        )
        return agent

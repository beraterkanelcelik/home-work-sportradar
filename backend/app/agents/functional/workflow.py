...
def _get_or_create_checkpointer() -> PostgresSaver:
    ...
    if checkpointer is None:
        ...
        if checkpointer is None:
            raise RuntimeError(
                "Database checkpointer unavailable - database may not be ready"
            )
    return checkpointer
...
def extract_tool_proposals(tool_calls: List[Dict[str, Any]]) -> List[ToolProposal]:
    ...
    for tc in tool_calls:
        ...
        if tool_name:
            proposals.append(ToolProposal(tool=tool_name, props=tool_args, query=""))
    return proposals
...
def request_tool_approvals(
    tool_calls: List[Dict[str, Any]], session_id: int
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"[HITL] [INTERRUPT] Requesting approval for {len(to_review)} tools: {[t['tool'] for t in to_review]} session={session_id}"
    )
    ...
    logger.info(
        f"[HITL] [RESUME] Interrupt returned with approval decisions: {len(decision.get('approvals', {}))} approvals session={session_id}"
    )
...
def request_plan_approval(plan: Dict[str, Any], session_id: int) -> Dict[str, Any]:
    ...
    logger.info(
        f"[PLAN_HITL] [INTERRUPT] Requesting approval for {len(plan_steps)} step plan: session={session_id}"
    )
    ...
    logger.info(
        f"[PLAN_HITL] [RESUME] Plan approval decision: approved={approved} session={session_id}"
    )
...
try:
    ...
    if _lazy_checkpointer is None:
        logger.debug(
            "Checkpointer not available at import time (database may not be ready), will be created on first use"
        )
except Exception as e:
...
def ai_agent_workflow(request: Union[AgentRequest, Command, Any]) -> AgentResponse:
    ...
    trace_id_for_langfuse = (
        getattr(request, "trace_id", None) if not isinstance(request, Command) else None
    )
    public_key = None
    secret_key = None
    if not isinstance(request, Command):
        public_key = getattr(request, "langfuse_public_key", None)
        secret_key = getattr(request, "langfuse_secret_key", None)
    if LANGFUSE_ENABLED and trace_id_for_langfuse and public_key and secret_key:
        try:
            ...
            if langfuse:
                ...
                flow = (
                    getattr(request, "flow", "main")
                    if not isinstance(request, Command)
                    else "main"
                )
                plan_steps = (
                    getattr(request, "plan_steps", None)
                    if not isinstance(request, Command)
                    else None
                )
                user_id = (
                ...
                logger.debug(
                    f"[LANGFUSE] Created workflow span for trace_id={trace_id_for_langfuse}"
                )
        except Exception as e:
            logger.warning(
                f"Failed to create Langfuse span for workflow: {e}", exc_info=True
            )
    ...
    try:
        ...
        if isinstance(request, Command):
            ...
            if hasattr(request, "resume") and isinstance(request.resume, dict):
                ...
                logger.error(
                    "[HITL] [RESUME] Command resume missing session_id in resume payload"
                )
            ...
            logger.info(
                f"[HITL] [RESUME] Command resume - session_id={current_session_id}"
            )
        else:
            ...
            current_run_id = getattr(
                request, "run_id", None
            )  # Correlation ID for /run polling
            current_parent_message_id = getattr(
        ...
        if isinstance(request, Command) and current_user_id is None and current_session_id:
            try:
                ...
                session = ChatSession.objects.only("user_id").get(id=current_session_id)
        ...
        thread_id = (
            f"chat_session_{current_session_id}"
            if current_session_id
            else f"user_{current_user_id}"
            if current_user_id
            else "default"
        )
        checkpoint_config = (
            get_checkpoint_config(current_session_id)
            if current_session_id
            else {"configurable": {"thread_id": thread_id}}
        )
        checkpoint_config["api_key"] = api_key
        ...
        if not isinstance(request, Command) and request.plan_steps:
            return _execute_plan_workflow(
                request, checkpoint_config, checkpointer, thread_id
            )
                ...
                if isinstance(msg, AIMessage) and hasattr(msg, "tool_calls") and msg.tool_calls:
                    ...
                    if isinstance(tool_calls_data, list) and tool_calls_data:
                        ...
                        approval_required_tools = [
                            ...
                            and tc.get("status")
                            in (None, "pending", "awaiting_approval")
                        ]
                        if approval_required_tools:
                            pending_tool_calls = tool_calls_data
                            logger.info(
                                "[HITL] [RESUME] Found pending tool_calls in last assistant message: "
                                f"{len(approval_required_tools)} tools awaiting approval"
                            )
                            break
        ...
        if not isinstance(request, Command) or pending_tool_calls is None:
            ...
            if not isinstance(request, Command):
                ...
                if (
                    ...
                    or messages[-1].content != request.query
                ):
            ...
            if pending_tool_calls is None:
                ...
                if messages:
                    ...
                    if last_ai_msg:
                        ...
                        if hasattr(last_ai_msg, "response_metadata"):
                            routing_agent = last_ai_msg.response_metadata.get(
                                "agent", "greeter"
                            )
                ...
                logger.info(
                    "[HITL] [RESUME] Reconstructed response from pending tool_calls: "
                    f"{len(pending_tool_calls)} tools, agent={routing_agent}"
                )
        ...
        if response and response.tool_calls:
            ...
            logger.info(
                f"[WORKFLOW] Tool partitioning: auto={len(auto_tools)} approval={len(approval_tools)} "
                f"manual={len(manual_tools)} session={current_session_id}"
            )
            ...
            if not isinstance(request, Command) and current_session_id:
                ...
                if any(
                    tool_requires_approval(tc.get("name") or tc.get("tool", ""))
                    for tc in response.tool_calls
                ):
                    try:
                        save_message_task(
                        ...
                        logger.info(
                            "[HITL] [STORE] Stored assistant message with "
                            f"{len([tc for tc in response.tool_calls if tool_requires_approval(tc.get('name') or tc.get('tool', ''))])} "
                            f"tools awaiting approval session={current_session_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"[HITL] [STORE] Failed to store assistant message before interrupt: {e}"
                        )
            ...
            if approval_tools:
                logger.info(
                    f"[HITL] [INTERRUPT] Requesting approval for {len(approval_tools)} tools requiring approval "
                    f"session={current_session_id}"
                )
                approval_decisions = request_tool_approvals(
                    approval_tools, current_session_id or 0
                )
                ...
                for tc in approval_tools:
                    ...
                    if approval.get("approved"):
                        ...
                        tc["status"] = "approved"
                        logger.info(
                            f"[HITL] Tool approved: tool={tool_name} tool_call_id={tool_call_id} "
                            f"session={current_session_id}"
                        )
                    else:
                        # Tool was rejected
                        tc["status"] = "rejected"
                        logger.info(
                            f"[HITL] Tool rejected: tool={tool_name} tool_call_id={tool_call_id} "
                            f"session={current_session_id}"
                        )
            ...
            ai_message_with_tool_calls = AIMessage(
                content=response.reply or "", tool_calls=tool_calls_with_ids
            )
            ...
            if tools_to_execute:
                logger.info(
                    f"[WORKFLOW] Executing {len(tools_to_execute)} tools: auto={len(auto_tools)} "
                    f"manual={len(manual_tools)} approved={len([tc for tc in approval_tools if tc.get('status') == 'approved'])} "
                    f"session={current_session_id}"
                )
                ...
                logger.info(
                    f"[WORKFLOW] Tool execution completed: {len(tool_results)} results"
                )
            ...
            for tc in tools_to_execute:
                ...
                if matching_result:
                    ...
                    tc["status"] = "error"
                    tc["error"] = (
                        "Tool execution failed - no result returned for tool_call_id="
                        f"{tool_call_id}"
                    )
            ...
            if approval_tools and current_run_id:
                try:
                    ...
                    if existing_message:
                        ...
                        logger.info(
                            f"[HITL] [DB_UPDATE] Updated message ID={existing_message.id} with completed tool statuses "
                            f"session={current_session_id}"
                        )
                except Exception as e:
                    logger.warning(
                        "[HITL] [DB_UPDATE] Failed to update message with completed tool statuses: "
                        f"{e} session={current_session_id}",
                        exc_info=True,
                    )
            ...
            if tool_results:
                ...
                for tr in tool_results:
                    ...
                    if not tool_call_id:
                        ...
                        logger.warning(
                            "[WORKFLOW] ToolResult missing tool_call_id, generated fallback: "
                            f"{tool_call_id} tool={tr.tool}"
                        )
                ...
                executed_tool_calls_with_status = (
                    response.tool_calls.copy() if response.tool_calls else []
                )
                ...
                logger.info(
                    f"[WORKFLOW] Invoking agent_with_tool_results_task for agent={refine_agent} with "
                    f"{len(tool_results)} tool results, messages_count={len(messages)}"
                )
                ...
                logger.info(
                    "[WORKFLOW] agent_with_tool_results_task returned: "
                    f"has_reply={bool(refined_response.reply)}, reply_preview="
                    f"{refined_response.reply[:50] if refined_response.reply else '(empty)'}..., "
                    f"tool_calls_count={len(refined_response.tool_calls) if refined_response.tool_calls else 0}"
                )
            ...
            rejected_tools = [
                tc for tc in approval_tools if tc.get("status") == "rejected"
            ]
            if rejected_tools and not tool_results:
                # If we have rejected tools and no tool results, create plan proposal
                logger.info(
                    f"[HITL] Creating plan proposal for {len(rejected_tools)} rejected tools "
                    f"session={current_session_id}"
                )
                # Get routing agent and query
                if "routing" in locals() and routing:
        ...
        if not response.tool_calls and executed_tool_calls_with_status:
            ...
            logger.info(
                f"Restored {len(final_tool_calls)} tool_calls in final response for streaming"
            )
        ...
        if trace_span:
            try:
                trace_span.update(output=None, level="ERROR", status_message=str(e))
            except Exception:
                pass
            finally:
                # Ensure span is ended
    ...
    if isinstance(chunk, dict):
        ...
        if any(key in chunk for key in ["agent_name", "reply", "tool_calls", "type"]):
            try:
                ...
                logger.info(
                    "[STREAM_CHUNK] Constructed AgentResponse from chunk dict: "
                    f"agent={response.agent_name}, has_reply={bool(response.reply)}"
                )
                return response
            except Exception as e:
                logger.warning(
                    "[STREAM_CHUNK] Failed to construct AgentResponse from chunk: "
                    f"{e}, chunk_keys={list(chunk.keys())}"
                )
...
def extract_interrupt_value(interrupt_raw: Any) -> Dict[str, Any]:
    ...
    logger.warning(
        "[HITL] [INTERRUPT] Could not extract interrupt value, using raw data: "
        f"{type(interrupt_raw)}"
    )
...
def partition_tools(
    tool_calls: List[Dict[str, Any]], agent_name: str
    ...
    try:
        ...
        try:
            ...
            if latest_plan_msg and latest_plan_msg.metadata:
                ...
                logger.debug(
                    f"[PLAN_PROGRESS] Updated plan progress: {current_step + 1}/{total_steps} "
                    f"session={session_id}"
                )
        except ImportError:
            # Message model not available (testing environment)
            logger.debug(
                "[PLAN_PROGRESS] Message model not available, skipping progress update"
            )
    except Exception as e:
...
def _execute_plan_workflow(
    ...
    public_key = getattr(request, "langfuse_public_key", None)
    secret_key = getattr(request, "langfuse_secret_key", None)
    if LANGFUSE_ENABLED and request.trace_id and public_key and secret_key:
        try:
            langfuse = get_langfuse_client_for_user(public_key, secret_key)
            if langfuse:
                # Create span within the trace hierarchy using trace_context
                plan_span = langfuse.start_observation(
                    ...
                    name="plan_execution",
                    metadata={
                        "plan_steps_count": len(request.plan_steps)
                ...
                logger.debug(
                    f"[LANGFUSE] Created plan execution span for trace_id={request.trace_id}"
                )
        except Exception as e:
            logger.warning(
                f"Failed to create Langfuse span for plan execution: {e}", exc_info=True
            )
    ...
    try:
        ...
        for step_index, step in enumerate(request.plan_steps or []):
            ...
            if request.session_id:
                _update_plan_progress(
                    request.session_id, step_index, len(request.plan_steps)
                )

            logger.info(
                f"[PLAN_EXEC] Step {step_index + 1}/{len(request.plan_steps)}: action={action}"
            )
            ...
            if action == "tool":
                ...
                try:
                    ...
                    logger.info(
                        f"[PLAN_EXEC] Executing tool: {tool_name} with agent {agent_name}"
                    )
                    tool_calls = [
                    ...
                    if tool_results:
                        ...
                        if tool_result.error:
                            logger.error(
                                f"[PLAN_EXEC] Tool failed at step {step_index + 1}: {tool_result.error}"
                            )
                            failed_step_index = step_index
                            results.append(
                                f"❌ Step {step_index + 1} failed: {tool_result.error}"
                            )
                        ...
                        if tool_result.output is not None:
                            logger.info(
                                f"[PLAN_EXEC] Refining results with agent {agent_name}"
                            )
                            ...
                            if agent_response.reply:
                                ...
                                logger.info(
                                    f"[PLAN_EXEC] Step {step_index + 1} completed successfully"
                                )

                except Exception as e:
                    # Exception during tool execution - STOP ON FAILURE
                    logger.error(
                        f"[PLAN_EXEC] Exception at step {step_index + 1}: {e}",
                        exc_info=True,
                    )
                    failed_step_index = step_index
        ...
        if request.session_id and final_text:
            final_response = AgentResponse(
                type="answer",
                reply=final_text,
                agent_name=request.plan_steps[0].get("agent", "greeter")
                if request.plan_steps
                else "greeter",
            )
            save_message_task(
            ...
            try:
                plan_span.update(
                    output={
                        ...
                        "tools_executed": len(raw_tool_outputs)
                        if raw_tool_outputs
                        else 0,
        ...
        if plan_span:
            try:
                plan_span.update(output=None, level="ERROR", status_message=str(e))
            except Exception:
                pass
            finally:
                # Ensure span is ended
...
async def ai_agent_workflow_events(
    request: Union[AgentRequest, Command],
        ...
        try:
            ...
            if public_key and secret_key:
                ...
                if langfuse_callback:
                    ...
                    logger.debug(
                        f"[LANGFUSE] Added CallbackHandler to workflow for trace_id={trace_id}"
                    )
        except Exception as e:
            logger.warning(
                f"Failed to add Langfuse CallbackHandler: {e}", exc_info=True
            )
            ...
            if LANGFUSE_ENABLED and trace_id:
                ...
                if langfuse:
                    ...
                    trace_context = prepare_trace_context(
                        ...
                        metadata={
                            "trace_id": trace_id
                        },  # Store trace_id in metadata for reference
                    )
                                ...
                                try:
                                    workflow_span.update(
                                        output={
                                            ...
                                            "agent_name": final_response_holder[
                                                0
                                            ].agent_name,
                                            "has_reply": bool(
                                                final_response_holder[0].reply
                                            ),
                                        }
                                    )
                                except Exception as e:
    ...
    logger.info(
        f"[EVENT_QUEUE] Started workflow thread, waiting for events (queue_size={event_queue.qsize()})"
    )
    ...
    while not workflow_done and timeout_count < max_timeout:
        try:
            ...
            try:
                ...
                if event_type != "token":
                    logger.debug(
                        f"[EVENT_QUEUE] Consumed event #{events_yielded} type={event_type} (queue_size={event_queue.qsize()})"
                    )
                yield event
                ...
                if event_type == "final":
                    workflow_done = True
                    logger.info(
                        f"[EVENT_QUEUE] Received final event, stopping queue consumption"
                    )
                    break
            except Empty:
                timeout_count += 1
                ...
                if not workflow_thread.is_alive():
                    logger.debug(
                        f"[EVENT_QUEUE] Workflow thread finished, checking for remaining events (queue_size={event_queue.qsize()})"
                    )
                    workflow_done = True
                    ...
                    try:
                        while True:
                            ...
                            event_type = event.get("type", "unknown")
                            logger.debug(
                                f"[EVENT_QUEUE] Draining event type={event_type} after thread completion"
                            )
                            yield event
                            if event_type == "final":
                                break
            ...
            logger.error(
                f"[EVENT_QUEUE] Error reading from event queue: {e}", exc_info=True
            )
            break
    ...
    if exception_holder[0]:
        yield {"type": "error", "error": str(exception_holder[0])}
        return
    ...
    if final_response:
        ...
        logger.error(
            f"[STREAM_ERROR] No final response extracted from stream chunks. This indicates a bug in extraction logic."
        )
        yield {
            "type": "error",

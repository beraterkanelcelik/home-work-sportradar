...
class SupervisorAgent(BaseAgent):
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                f"Failed to create structured output LLM: {e}, falling back to regular LLM"
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if latest_message:
                ...
                if any(
                    latest_message.startswith(keyword) for keyword in search_keywords
                ):
                    logger.info(
                        f"Keyword-based routing: routing '{latest_message[:50]}...' to search agent"
                    )
                    return RoutingDecisionModel(
                        agent="search",
                        confidence=1.0,
            ...
            if self.routing_llm:
                ...
                try:
                    decision = self.routing_llm.invoke(
                        [system] + messages, **invoke_kwargs
                    )
                except Exception as e:
                    logger.warning(
                        f"Structured output parsing failed: {e}, falling back to regular invoke"
                    )
                    decision = None
            else:
                decision = None
            ...
            if decision is None:
                ...
                if (
                    ...
                    logger.warning(
                        "Supervisor response is empty or invalid, defaulting to greeter"
                    )
                    return RoutingDecisionModel(
                        agent="greeter",
                        confidence=0.5,

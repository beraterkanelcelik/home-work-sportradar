...
async def stream_agent(request):
    ...
    if current_count >= MAX_CONCURRENT_STREAMS_PER_USER:
        logger.warning(
            f"Rate limit exceeded for user {user.id}: {current_count}/{MAX_CONCURRENT_STREAMS_PER_USER} streams"
        )
        return JsonResponse(
            {
    ...
    try:
        ...
        if flow != "plan" and message:
            ...
            try:
                user_message = await sync_to_async(add_message)(
                    chat_session_id, "user", message
                )
                user_message_id = user_message.id
                logger.info(
                    f"[MESSAGE_SAVE] Saved user message ID={user_message.id} session={chat_session_id} content_preview={message[:50]}..."
                )
            except Exception as e:
                logger.error(f"Error saving user message: {e}", exc_info=True)
                return JsonResponse(
                    {"error": "Failed to save user message"}, status=500
                )
        ...
        async def event_stream():
            ...
            try:
                ...
                if use_redis:
                    try:
                        ...
                        if confirm_msg and confirm_msg["type"] == "subscribe":
                            ...
                            logger.warning(
                                f"Unexpected subscription confirmation: {confirm_msg}"
                            )
                        ...
                        if user_message_id is not None:
                            try:
                                ...
                                await redis_client.publish(
                                    channel, event_json.encode("utf-8")
                                )
                                logger.info(
                                    f"[MESSAGE_SAVED_EVENT] Emitted user message_saved event db_id={user_message_id} session={chat_session_id}"
                                )
                            except Exception as e:
                                logger.warning(
                                    f"Failed to emit message_saved event for user message: {e}"
                                )

                        # Get or create workflow and send message signal
                        try:
                            # Generate unique run_id for this request
                            # This is CRITICAL for checkpoint isolation - each request gets its own checkpoint thread
                            run_id = str(uuid.uuid4())
                            logger.info(
                                f"[RUN_ID] Generated run_id={run_id} for session={chat_session_id}"
                            )
                            ...
                            workflow_state = {
                                ...
                                "flow": flow,
                                "tenant_id": tenant_id,
                                "run_id": run_id,  # CRITICAL: For checkpoint isolation in HITL interrupt/resume
                            ...
                            workflow_handle = await get_or_create_workflow(
                                user.id, chat_session_id, initial_state=workflow_state
                            )
                            logger.info(
                                f"Using Temporal workflow {workflow_handle.id} for chat {chat_session_id} (signal sent automatically)"
                            )
                        except Exception as e:
                            logger.error(
                                f"Failed to start Temporal workflow: {e}", exc_info=True
                            )
                        ...
                        try:
                            ...
                            while reconnect_attempt < MAX_RECONNECT_ATTEMPTS:
                                try:
                                    ...
                                    if reconnect_attempt > 0:
                                        logger.info(
                                            f"Reconnecting to Redis channel {channel} (attempt {reconnect_attempt}/{MAX_RECONNECT_ATTEMPTS})"
                                        )
                                        pubsub = redis_client.pubsub()
                                        await pubsub.subscribe(channel)
                                        ...
                                        if (
                                            confirm_msg
                                            and confirm_msg["type"] == "subscribe"
                                        ):
                                            logger.info(
                                                f"Reconnected to Redis channel: {channel}"
                                            )
                                        else:
                                            logger.warning(
                                                f"Unexpected subscription confirmation after reconnect: {confirm_msg}"
                                            )
                                    ...
                                    async for msg in pubsub.listen():
                                        ...
                                        if msg["type"] == "message":
                                            try:
                                                ...
                                                logger.warning(
                                                    f"Failed to decode Redis message: {e}"
                                                )
                                                continue
                                            except Exception as e:
                                                logger.error(
                                                    f"Error processing Redis message: {e}",
                                                    exc_info=True,
                                                )
                                                continue
                                        elif (
                                            msg["type"] == "unsubscribe"
                                            or msg["type"] == "punsubscribe"
                                        ):
                                    ...
                                    if (
                                        ...
                                        logger.warning(
                                            f"Redis connection error, reconnecting in {delay}s (attempt {reconnect_attempt}/{MAX_RECONNECT_ATTEMPTS}): {e}"
                                        )
                            ...
                            logger.info(
                                f"Redis subscription cancelled for channel {channel}"
                            )
                            raise
                        except Exception as e:
                            ...
                            if reconnect_attempt >= MAX_RECONNECT_ATTEMPTS:
                                logger.error(
                                    f"Redis subscription failed after {MAX_RECONNECT_ATTEMPTS} reconnection attempts: {e}",
                                    exc_info=True,
                                )
                            else:
                                logger.error(
                                    f"Error in Redis subscription loop: {e}",
                                    exc_info=True,
                                )
                        ...
                        logger.warning(
                            f"Redis streaming failed, falling back to direct: {redis_error}",
                            exc_info=True,
                        )
                ...
                logger.error(
                    f"Error in agent stream for user {user.id}, session {chat_session_id}: {e}",
                    exc_info=True,
                )
                # Send error event
                    ...
                    try:
                        ...
                        if not loop.is_closed():
                            try:
                                ...
                                await pubsub.close()
                                logger.debug(
                                    f"Cleaned up Redis pubsub for channel {channel or 'unknown'}"
                                )
                            except (RuntimeError, asyncio.CancelledError) as e:
                                logger.debug(
                                    f"Event loop closing, skipping pubsub cleanup: {e}"
                                )
                        else:
                            logger.debug(
                                "Event loop is closed, skipping pubsub cleanup"
                            )
                    except RuntimeError:
                        # No running loop - skip cleanup (non-critical)
                        # This is safe: pubsub will be cleaned up when the process exits
                        logger.debug(
                            "No running event loop for pubsub cleanup, skipping (non-critical)"
                        )
                    except Exception as e:
                        logger.debug(
        ...
        response = StreamingHttpResponse(
            event_stream_with_metadata(), content_type="text/event-stream"
        )
...
async def approve_tool(request):
    ...
    try:
        ...
        if not chat_session_id:
            logger.warning(
                f"Missing chat_session_id in approve_tool request from user {user.id}"
            )
            return JsonResponse({"error": "chat_session_id is required"}, status=400)

        if not resume:
            logger.warning(
                f"Missing resume in approve_tool request from user {user.id}"
            )
        ...
        if not session:
            logger.warning(
                f"[HITL] [SECURITY] Session ownership validation failed: user={user.id} session={chat_session_id}"
            )
            return JsonResponse(
        ...
        logger.info(
            f"[HITL] [RESUME] Resume request received: user={user.id} session={chat_session_id} resume_keys={list(resume.keys()) if isinstance(resume, dict) else 'N/A'}"
        )
                    ...
                    if tool_calls:
                        ...
                        for tc in tool_calls:
                            ...
                            if approval_decision.get("approved", False):
                                ...
                                logger.info(
                                    f"[HITL] [DB_UPDATE] Marked tool {tc.get('name') or tc.get('tool')} as approved in message ID={latest_assistant_msg.id} session={chat_session_id}"
                                )
                                ...
                                updated_tool_calls.append(updated_tc)
                                logger.info(
                                    f"[HITL] [DB_UPDATE] Marked tool {tc.get('name') or tc.get('tool')} as rejected in message ID={latest_assistant_msg.id} session={chat_session_id}"
                                )
                            else:
                        ...
                        logger.info(
                            f"[HITL] [DB_UPDATE] Updated message ID={latest_assistant_msg.id} with approval decisions session={chat_session_id}"
                        )
            except Exception as e:
                logger.warning(
                    f"[HITL] [DB_UPDATE] Failed to update message with approval decisions: {e} session={chat_session_id}",
                    exc_info=True,
                )
        ...
        try:
            ...
            try:
                ...
                if description.status.name != "RUNNING":
                    logger.warning(
                        f"[HITL] [RESUME] Workflow {workflow_id} is not running (status: {description.status.name})"
                    )
                    return JsonResponse(
                        {
                            "success": False,
            ...
            await workflow_handle.signal("resume", args=(resume,))
            logger.info(
                f"[HITL] [RESUME] Sent resume signal to workflow: session={chat_session_id}"
            )
        except Exception as e:
            logger.error(
                f"[HITL] [RESUME] Failed to send resume signal to workflow: error={e} session={chat_session_id}",
                exc_info=True,
            )
            return JsonResponse(
                {"success": False, "error": f"Failed to send resume signal: {str(e)}"},
                status=500,
            )
        ...
        logger.info(
            f"[HITL] [RESUME] Approval signal sent successfully, returning immediately: session={chat_session_id}"
        )
        return JsonResponse(
            {
                "success": True,
...
async def approve_plan(request):
    ...
    try:
        ...
        if not chat_session_id:
            logger.warning(
                f"Missing chat_session_id in approve_plan request from user {user.id}"
            )
            return JsonResponse({"error": "chat_session_id is required"}, status=400)

        if not resume:
            logger.warning(
                f"Missing resume in approve_plan request from user {user.id}"
            )
            return JsonResponse({"error": "resume is required"}, status=400)
        ...
        if not session:
            logger.warning(
                f"[PLAN_HITL] [SECURITY] Session ownership validation failed: user={user.id} session={chat_session_id}"
            )
            return JsonResponse(
                {"error": "Chat session not found or access denied"}, status=404
            )
        ...
        try:
            ...
            try:
                ...
                if description.status.name != "RUNNING":
                    logger.warning(
                        f"[PLAN_HITL] [RESUME] Workflow {workflow_id} is not running (status: {description.status.name})"
                    )
                    return JsonResponse(
                        {
                            "success": False,
                ...
                logger.error(
                    f"[PLAN_HITL] [RESUME] Workflow {workflow_id} not found: {e}"
                )
                return JsonResponse(
                    {
                        "success": False,
            ...
            await workflow_handle.signal("resume", args=(resume,))
            logger.info(
                f"[PLAN_HITL] [RESUME] Sent plan approval signal to workflow: session={chat_session_id} approved={approved}"
            )

        except Exception as e:
            logger.error(
                f"[PLAN_HITL] [RESUME] Failed to send resume signal to workflow: error={e} session={chat_session_id}",
                exc_info=True,
            )
            return JsonResponse(
                {"success": False, "error": f"Failed to send resume signal: {str(e)}"},
        ...
        # Return immediately - workflow will process asynchronously
        logger.info(
            f"[PLAN_HITL] [RESUME] Plan approval signal sent successfully: session={chat_session_id}"
        )
        return JsonResponse(
            {
                "success": True,
...
async def list_scout_reports(request):
    ...
    Query params:
    - player_id: Filter by player UUID (optional)
    ...
    user = await get_current_user_async(request)
    if not user:
        logger.warning("Unauthenticated request to list_scout_reports endpoint")
        return JsonResponse({"error": "Authentication required"}, status=401)
    ...
    try:
        ...
        if hasattr(request, "GET") and isinstance(request.GET, QueryDict):
            player_id = request.GET.get("player_id")
        else:
            player_id = None

        logger.info(f"Listing scout reports for user {user.id}, player_id={player_id}")

        # Build query with filters
        reports_query = ScoutingReport.objects.filter(player__owner=user)

        if player_id:
            try:
                from uuid import UUID as uuid_module

                player_uuid = uuid_module.UUID(player_id)
                reports_query = reports_query.filter(player_id=player_uuid)
            except ValueError:
                logger.warning(f"Invalid player_id format: {player_id}")
                return JsonResponse({"error": "Invalid player_id format"}, status=400)

        # Order by most recent first
        reports_query = reports_query.select_related("player").order_by("-created_at")

        # Get reports
        reports_list = await sync_to_async(lambda: list(reports_query.all()[:100]))()

        # Serialize to response format
        reports_data = []
        for report in reports_list:
            # Build physical object from individual fields (all optional)
            physical = {}
            if report.player.height_cm is not None:
                physical["height_cm"] = report.player.height_cm
            if report.player.weight_kg is not None:
                physical["weight_kg"] = report.player.weight_kg
            if report.player.measurements:
                physical["measurements"] = report.player.measurements

            # Build scouting object from individual fields (all optional)
            scouting = {}
            if report.player.strengths:
                scouting["strengths"] = report.player.strengths
            if report.player.weaknesses:
                scouting["weaknesses"] = report.player.weaknesses
            if report.player.style_tags:
                scouting["style_tags"] = report.player.style_tags
            if report.player.risk_notes:
                scouting["risk_notes"] = report.player.risk_notes
            if report.player.role_projection:
                scouting["role_projection"] = report.player.role_projection

            reports_data.append(
                {
                    "id": str(report.id),
                    "player": {
                        "id": str(report.player.id),
                        "display_name": report.player.display_name,
                        "sport": report.player.sport or "unknown",
                        "positions": report.player.positions or [],
                        "teams": report.player.teams or [],
                        "league": report.player.league or "",
                        "aliases": report.player.aliases or [],
                        "physical": physical if physical else None,
                        "scouting": scouting if scouting else None,
                    },
                    "report_text": report.report_text or "",
                    "report_summary": report.report_summary or "",
                    "coverage": report.coverage,
                    "created_at": report.created_at.isoformat(),
                    "run_id": report.run_id or "",
                    "request_text": report.request_text or "",
                    "source_doc_ids": report.source_doc_ids or [],
                }
            )

        logger.info(f"Returning {len(reports_data)} scout reports for user {user.id}")
...
@csrf_exempt
@require_http_methods(["POST"])
async def approve_player(request):
    """
    Resume interrupted scouting workflow with player approval decision (LangGraph native interrupt pattern).
    ...
    user = await get_current_user_async(request)
    if not user:
        logger.warning("Unauthenticated request to approve_player endpoint")
        return JsonResponse({"error": "Authentication required"}, status=401)
    ...
    try:
        ...
        if not chat_session_id:
            logger.warning(
                f"Missing chat_session_id in approve_player request from user {user.id}"
            )
            return JsonResponse({"error": "chat_session_id is required"}, status=400)

        if not resume:
            logger.warning(
                f"Missing resume in approve_player request from user {user.id}"
            )
            return JsonResponse({"error": "resume is required"}, status=400)
        ...
        if not session:
            logger.warning(
                f"[PLAYER_HITL] [SECURITY] Session ownership validation failed: user={user.id} session={chat_session_id}"
            )
            return JsonResponse(
                {"error": "Chat session not found or access denied"}, status=404
            )
        ...
        try:
            ...
            try:
                ...
                if description.status.name != "RUNNING":
                    logger.warning(
                        f"[PLAYER_HITL] [RESUME] Workflow {workflow_id} is not running (status: {description.status.name})"
                    )
                    return JsonResponse(
                        {
                            "success": False,
                ...
                logger.error(
                    f"[PLAYER_HITL] [RESUME] Workflow {workflow_id} not found: {e}"
                )
                return JsonResponse(
                    {
                        "success": False,
            ...
            # Send resume signal with player approval decision
            await workflow_handle.signal("resume", args=(resume,))
            logger.info(
                f"[PLAYER_HITL] [RESUME] Sent player approval signal to workflow: session={chat_session_id} action={action}"
            )

        except Exception as e:
            logger.error(
                f"[PLAYER_HITL] [RESUME] Failed to send resume signal to workflow: error={e} session={chat_session_id}",
                exc_info=True,
            )
            return JsonResponse(
                {"success": False, "error": f"Failed to send resume signal: {str(e)}"},
        ...
        # Return immediately - workflow will process asynchronously
        logger.info(
            f"[PLAYER_HITL] [RESUME] Player approval signal sent successfully: session={chat_session_id}"
        )
        return JsonResponse(
            {
                "success": True,
                "status": action,
                "message": f"Player {action}ed. Workflow will {'save to database' if action == 'approve' else 'stop'}.",
                "session_id": chat_session_id,
                "messages_endpoint": f"/api/chats/{chat_session_id}/messages/",
            }
        )
        ...
        logger.error(f"Invalid JSON in approve_player request: {e}")
        return JsonResponse({"error": "Invalid JSON"}, status=400)
    except Exception as e:
        logger.error(f"Unexpected error in approve_player endpoint: {e}", exc_info=True)
        return JsonResponse({"error": str(e)}, status=500)
